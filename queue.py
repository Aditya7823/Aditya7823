...
"Queue vs Deque": {
  ...
  "Implement a circular queue of size {size} initialized with {data_type}: {initial_elements}. After executing {operations}, what are the front and rear values?",
...
# Streamlit UI
st.title("🔁 Queue Problem Generator")

st.markdown("Generate creative and random queue-based data structure problems for practice or teaching!")

difficulty = st.selectbox("Select difficulty level:", difficulty_levels)

if st.button("🎲 Generate Queue Question"):
    st.markdown("### 📘 Your Queue Problem:")
    st.success(generate_queue_question(difficulty.lower()))