...
# Function to generate a question with plot or table
def get_question(topic, subtopic, difficulty, displayed_questions):
    ...
    params = {
        ...
        "m": random.randint(5, 10),  # For data tables
        "s": random.uniform(0.1, 2)  # Slope for sigmoid
    }
    params.update({key: random.choice(values) for key, values in options.items() if f"{{{key}}}" in question})
    
    # Generate plot or table based on template
    fig = None
    table_data = None
    ...
    if "scatter plot below" in question:
        ...
        f_features = params["f"]
        data = {
            f"Feature_{i+1}": np.random.uniform(-5, 5, m_samples) for i in range(f_features)
        }
    ...
    # Extract dependency parse as HTML
    dependency_html = displacy.render(doc, style="dep", options={"compact": True}, page=False)

    # Print results in terminal
    print("\n=== Tokenization, POS Tagging, and Dependency Parsing ===")
    for token, pos, dep in tokens:
        print(f"Token: {token:15} | POS: {pos:10} | Dependency: {dep}")
    print("=========================================================\n")

    # Generate ASCII Dependency Tree
    print("\n=== Dependency Tree ===")

    def print_tree(token, prefix="", is_last=True):
        """Recursively print the dependency tree in the terminal."""
        branch = "└── " if is_last else "├── "
        print(prefix + branch + f"[{token.dep_}] {token.text}")

        prefix += "    " if is_last else "│   "
        children = list(token.children)  # Convert generator to list
    ...
    print(f"[{root.dep_}] {root.text}")  # Root node
    children = list(root.children)  # Convert generator to list
    for i, child in enumerate(children):
        print_tree(child, "", i == len(children) - 1)

    print("=======================\n")

    return {"question": question, "answer": "Sample answer for demonstration purposes.", "plot": fig, "table": table_data, "tokens": tokens, "dependency_html": dependency_html}


# Adjust layout: small padding on top, left, and right
st.markdown("""
    <style>
        /* Add small padding on the top, left, and right */
        .block-container {
        ...
        /* Ensure dependency parser container scrolls */
        .dependency-parser {
            width: 100% !important;
            max-width: 100% !important;
            height: 300px; /* Set a fixed height with scrolling */
        ...
        .dataframe {
        max-width: 500px !important;  /* Adjust table width */
        overflow-x: auto !important;
        }
    </style>
...
with col2:
    if st.session_state.current_question:
        # Display tokenization, POS tagging, and dependency parsing
        st.write("### Tokenization, POS Tagging, and Dependency Parsing:")
        tokens_df = pd.DataFrame(st.session_state.current_question["tokens"], columns=["Token", "POS", "Dependency"])
        st.table(tokens_df)

with col3:
    if st.session_state.current_question:
        # Display dependency parsing visualization with scrolling
        st.write("### Dependency Parsing:")
        # st.markdown('<div class="dependency-parser">', unsafe_allow_html=True)
        
        st.components.v1.html(st.session_state.current_question["dependency_html"], height=300, scrolling=True)
...
if uploaded_file is not None:
    ...
    try:
        ...
        st.write("Please ensure Tesseract OCR is installed correctly.")